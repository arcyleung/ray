...
from ray.util.collective.collective_group import hccl_util
...
class Rendezvous:
    ...
    To initialize an HCCL collective communication group, different
    actors/tasks spawned in Ray in a collective group needs to meet
    each other to synchronize the HCCLUniqueID. This class guarantees
    they meet via the HCCLUniqueIDStore, initialized on the rank=0
    process.

    Args:
        store_key: the unique store key, usually as a concatanation
            of group_name and communicator key. See `get_hccl_communicator`
            for more details.
    """
    ...
    def __init__(self, store_key):
        if not store_key:
            raise ValueError(
                ...
                "docstring of `get_hccl_communicator` for details."
            )
    ...
    def get_hccl_id(self, timeout_s=180):
        """Get the HCCLUniqueID from the store through Ray.

        Args:
            timeout_s: timeout in seconds.

        Return:
            uid: the HCCLUniqueID if successful.
        """
        if not self._store:
        ...
        while elapsed < timeout_delta:
            ...
            break
        if not uid:
            raise RuntimeError("Unable to get the HCCLUniqueID from the store.")
        return uid
...
class HCCLGroup(BaseGroup):
    def __init__(self, world_size, rank, group_name):
        ...
        if hccl_util.get_hccl_build_version() < 2000:
            raise RuntimeError("HCCL in Ray requires CANN >= 8.0.0")
        if hccl_util.get_hccl_runtime_version() < 2704:
            logger.warning("HCCL send/recv calls requires HCCL>=8.0.0")

    def destroy_group(self):
        """Destroy the group and release HCCL communicators."""
    ...
    def allreduce(self, tensors, allreduce_options=AllReduceOptions()):
        ...
        def collective_fn(input_tensor, output_tensor, comm, stream):
            comm.allReduce(
                hccl_util.get_tensor_ptr(input_tensor),
                hccl_util.get_tensor_ptr(output_tensor),
                hccl_util.get_tensor_n_elements(input_tensor),
                hccl_util.get_hccl_tensor_dtype(input_tensor),
                hccl_util.get_hccl_reduce_op(allreduce_options.reduceOp),
                stream.ptr,
            )
    ...
    def barrier(self, barrier_options=BarrierOptions()):
        ...
        if self._used_npu_indices:
            ...
            devices = list(range(hccl_util.get_num_npus()))
        barrier_tensors = [None] * len(devices)
        for i, d in enumerate(devices):
            with hccl_util.Device(d):
                # Arthur: Ascend equivalent?
    ...
    def reduce(self, tensors, reduce_options=ReduceOptions()):
        ...
        def collective_fn(input_tensor, output_tensor, comm, stream):
            comm.reduce(
                ...
                hccl_util.get_tensor_n_elements(input_tensor),
                hccl_util.get_hccl_tensor_dtype(input_tensor),
                hccl_util.get_hccl_reduce_op(reduce_options.reduceOp),
                root_rank,
                stream.ptr,
            )
        ...
        def postprocess_fn(stream):
            ...
            for i, tensor_list in enumerate(tensor_lists):
                for j, tensor in enumerate(tensor_list):
                    hccl_util.copy_tensor(tensor, output_flattened[i][j])
    ...
    def reducescatter(
        ...
        def collective_fn(input_tensor, output_tensor, comm, stream):
            comm.reduceScatter(
                hccl_util.get_tensor_ptr(input_tensor),
                hccl_util.get_tensor_ptr(output_tensor),
                hccl_util.get_tensor_n_elements(output_tensor),
                hccl_util.get_hccl_tensor_dtype(output_tensor),
                hccl_util.get_hccl_reduce_op(reducescatter_options.reduceOp),
                stream.ptr,
            )
        ...
        def preprocess_fn(stream):
            for i, tensor_list in enumerate(tensor_lists):
                for j, tensor in enumerate(tensor_list):
                    hccl_util.copy_tensor(input_flattened[i][j], tensor)
    ...
    def send(self, tensors, send_options=SendOptions()):
        ...
        def p2p_fn(tensor, comm, stream, peer):
            comm.send(
                hccl_util.get_tensor_ptr(tensor),
                send_options.n_elements
                if send_options.n_elements > 0
                else hccl_util.get_tensor_n_elements(tensor),
                hccl_util.get_hccl_tensor_dtype(tensor),
                peer,
                stream.ptr,
            )
    ...
    def _get_hccl_collective_communicator(self, comm_key, device_list):
        """Create or retrieve an HCCL communicator from cache.
        ...
        Returns:
            communicator: the HCCL communicator corresponded to the devices.
        """
        if not comm_key:
        ...
        group_key = self._generate_group_key(comm_key)
        if self.rank == 0:
            hccl_uid = self._generate_hccl_uid(group_key)
        else:
            rendezvous = Rendezvous(group_key)
            rendezvous.meet()
            hccl_uid = rendezvous.get_hccl_id()
        ...
        hccl_util.groupStart()
        for i, device in enumerate(device_list):
            actual_rank = self.rank * len(device_list) + i
            with hccl_util.Device(device):
                comms[i] = hccl_util.create_hccl_communicator(
                    actual_world_size, hccl_uid, actual_rank
                )
    ...
    def _sync_streams(device_list, events, streams):
        """Let HCCL streams wait for current streams for every device."""
        # TODO(Fu): recordStream besides calling this function?
        if ENV.HCCL_USE_MULTISTREAM.val:
            for i, device in enumerate(device_list):
                with hccl_util.Device(device):
                    events[i].record(cupy.cuda.get_current_stream())
                    streams[i].wait_event(events[i])

    def _get_hccl_p2p_communicator(self, comm_key, my_gpu_idx, peer_rank, peer_gpu_idx):
        ...
        with hccl_util.Device(my_gpu_idx):
            comm = hccl_util.create_hccl_communicator(2, hccl_uid, my_p2p_rank)
    ...
    def _generate_hccl_uid(self, key):
        """Generate an HCCL unique ID for initializing communicators.

        The method will also create a KV store using Ray named actor and store
        the HCCLUniqueID in the store. The store needs to be garbage collected
        ...
        Returns:
            HCCLUniqueID (str): HCCL unique ID.
        """
        group_uid = hccl_util.get_hccl_unique_id()
        store_name = get_store_name(key)
        # Avoid a potential circular dependency in ray/actor.py
        from ray.util.collective.util import HCCLUniqueIDStore

        store = HCCLUniqueIDStore.options(name=store_name, lifetime="detached").remote(
            store_name
        )
    ...
    def _collective(
        ...
        devices = hccl_util.get_tensor_device_list(input_tensors)
        ...
        # usages on hcclStreams finish. This can be achieved by calling
        # c10::cuda::CUDACachingAllocator::recordStream, which remembers the
        # usage stream (hcclStream), creates an event on the usage stream
    ...
    def _point2point(self, tensors, p2p_fn, peer_rank: int, peer_gpu_idx: int):
        ...
        if hccl_util.get_hccl_runtime_version() < 2704:
            raise RuntimeError(
                "P2p send/recv requires HCCL >= 2.7.4. "
                "Got '{}'.".format(hccl_util.get_hccl_runtime_version())
            )
        _check_gpu_tensors(tensors)

        # we currently only support single device to single device send/recv.
        assert len(tensors) == 1
        my_gpu_idx = hccl_util.get_tensor_device(tensors[0])
...
def _flatten_for_scatter_gather(tensor_list, copy=False):
    ...
    t = tensor_list[0]
    buffer_shape = [len(tensor_list)] + hccl_util.get_tensor_shape(t)
    ...
    if torch_available():
        ...
        dtype = hccl_util.get_cupy_tensor_dtype(t)
        device = hccl_util.get_tensor_device(t)
        with hccl_util.Device(device):
            buffer = cupy.empty(buffer_shape, dtype=dtype)

    if copy:
        for i, tensor in enumerate(tensor_list):
            hccl_util.copy_tensor(buffer[i], tensor)
    return buffer
...
def _check_inputs_compatibility_for_scatter_gather(tensors, tensor_lists):
    ...
    dtype = hccl_util.get_hccl_tensor_dtype(tensors[0])
    shape = hccl_util.get_tensor_shape(tensors[0])
    for i, tensor_list in enumerate(tensor_lists):
        # check all tensor in `tensors` match.
        dt = hccl_util.get_hccl_tensor_dtype(tensors[i])
        if dt != dtype:
            raise RuntimeError(
        ...
        # exact shape match.
        s = hccl_util.get_tensor_shape(tensors[i])
        if s != shape:
            raise RuntimeError(
                "All tensor operands to scatter/gather must "
        ...
        for t in tensor_lists[i]:
            # check dtype
            dt = hccl_util.get_hccl_tensor_dtype(t)
            if dt != dtype:
                raise RuntimeError(
                    "All tensor operands to scatter/gather must "
                    "have the same dtype. Got '{}' and '{}'.".format(dt, dtype)
                )
            s = hccl_util.get_tensor_shape(t)
            if s != shape:
                raise RuntimeError(
                    "All tensor operands to scatter/gather must "
                    "have the same shape. Got '{}' and '{}'.".format(s, shape)
...
def _check_gpu_tensors(tensors):
    ...
    if len(tensors) > hccl_util.get_num_gpus():
        raise RuntimeError(
            "Tensor list cannot be larger than the number"
            "of available GPUs. Got {} > {}.".format(
                len(tensors), hccl_util.get_num_gpus()
            )
        )
    t0 = tensors[0]
    dt = hccl_util.get_hccl_tensor_dtype(t0)
    s = hccl_util.get_tensor_shape(t0)
    d = hccl_util.get_tensor_device(t0)
    for i, t in enumerate(tensors):
        ...
        dtype = hccl_util.get_hccl_tensor_dtype(t)
        if dt != dtype:
            raise RuntimeError(
                "Tensors must have identical dtype. Got: '{}'.".format(dtype)
            )
        shape = hccl_util.get_tensor_shape(t)
        if s != shape:
            raise RuntimeError(
