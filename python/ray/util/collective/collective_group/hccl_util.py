...
try:
    ...
    # from cupy.cuda import hccl
    # from cupy.cuda import Device  # noqa: F401
    # from cupy.cuda.hccl import get_version
    # from cupy.cuda.hccl import get_build_version
    # from cupy.cuda.hccl import NcclCommunicator
    # from cupy.cuda.hccl import groupStart  # noqa: F401
    # from cupy.cuda.hccl import groupEnd  # noqa: F401
except ImportError:
    raise ImportError("NCCL in Ray requires pyacl being available!")
...
NUMPY_NCCL_DTYPE_MAP = {
    # INT types
    numpy.int_: hccl.NCCL_INT64,
    numpy.uint8: hccl.NCCL_UINT8,
    numpy.uint32: hccl.NCCL_UINT32,
    numpy.uint64: hccl.NCCL_UINT64,
    numpy.int8: hccl.NCCL_INT8,
    numpy.int32: hccl.NCCL_INT32,
    numpy.int64: hccl.NCCL_INT64,
    # FLOAT types
    numpy.half: hccl.NCCL_HALF,
    numpy.float16: hccl.NCCL_FLOAT16,
    numpy.float32: hccl.NCCL_FLOAT32,
    numpy.float64: hccl.NCCL_FLOAT64,
    numpy.double: hccl.NCCL_DOUBLE,
}
...
if torch_available():
    ...
    TORCH_NCCL_DTYPE_MAP = {
        torch.bool: hccl.NCCL_INT8,
        # INT types
        torch.int: hccl.NCCL_INT,
        torch.uint8: hccl.NCCL_UINT8,
        torch.int8: hccl.NCCL_INT8,
        torch.int32: hccl.NCCL_INT32,
        torch.int64: hccl.NCCL_INT64,
        torch.long: hccl.NCCL_INT64,
        # FLOAT types
        torch.half: hccl.NCCL_HALF,
        torch.float: hccl.NCCL_FLOAT,
        torch.float16: hccl.NCCL_FLOAT16,
        torch.float32: hccl.NCCL_FLOAT32,
        torch.float64: hccl.NCCL_FLOAT64,
        torch.double: hccl.NCCL_DOUBLE,
    }

    # Older versions of cupy don't support bfloat16.
    if hasattr(hccl, "NCCL_BFlOAT16"):
        TORCH_NCCL_DTYPE_MAP[torch.bfloat16] = hccl.NCCL_BFLOAT16
...
def get_hccl_build_version():
    return get_build_version()


def get_hccl_runtime_version():
    return get_version()


def get_hccl_unique_id():
    return hccl.get_unique_id()


def create_hccl_communicator(world_size, hccl_unique_id, rank):
    """Create an NCCL communicator using NCCL APIs.

    Args:
        world_size: the number of processes of this communicator group.
        hccl_unique_id: the NCCLUniqueID for this group.
        rank: the rank of this process.
    Returns:
        comm (hccl.hcclComm_t): an NCCL communicator.
    """
    comm = NcclCommunicator(world_size, hccl_unique_id, rank)
    return comm


def get_hccl_reduce_op(reduce_op):
    """Map the reduce op to NCCL reduce op type.

    Args:
        reduce_op: ReduceOp Enum (SUM/PRODUCT/MIN/MAX).
    Returns:
        (hccl.hcclRedOp_t): the mapped NCCL reduce op.
    """
